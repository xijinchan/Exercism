...
def penalty(current_player, opponent, central_pile, no_of_cards, cards_played, no_of_tricks, finished, player_a, player_b, game_states, penalty_previously, turn, loop_detected):
    ...
    for index in range(payment_cards.index(current_payment_card) + 1):
        if len(opponent) == 0: break
        next_card = opponent.pop(0)
        central_pile.append(next_card)
        cards_played += 1
        if check_payment_card(central_pile) is True:
            interrupted = True
    ...
    if interrupted is False:
        ...
        game_state = (['N' if card not in 'JQKA' else card for card in player_a],
                ['N' if card not in 'JQKA' else card for card in player_b],
                turn)
...
def simulate_game(player_a, player_b):
    ...
    # for turn_counterin range(no_of_cards * 69):
    while not finished and not loop_detected and iteration < max_iterations:
        iteration += 1
        
        turn = (turn_counter+ penalty_previously) % 2
