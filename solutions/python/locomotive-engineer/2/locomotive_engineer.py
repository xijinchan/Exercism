...
def fix_list_of_wagons(each_wagons_id, missing_wagons):
    ...
    :return: list - list of wagons.
    """
    wagons = get_list_of_wagons(each_wagons_id)[0]
    a, b, c, *remaining = wagons
    output = [c, *missing_wagons, *remaining, a, b]
    return output
...
def add_missing_stops(route, **kwargs):
    ...
    """
    output = {**route, 'stops': list(kwargs.values())}
    return output
...
def fix_wagon_depot(wagons_rows):
    ...
    """
    transposed = [list(x) for x in zip(*wagons_rows)]
    return transposed