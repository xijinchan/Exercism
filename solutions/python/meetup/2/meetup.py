...
def meetup(year, month, week, day_of_week):    
    ...
    if week == 'first': day = 1 + ((day_index - first_day) % 7)
    if week == 'second': day = 8 + ((day_index - first_day) % 7)
    if week == 'third': day = 15 + ((day_index - first_day) % 7)
    if week == 'fourth': day = 22 + ((day_index - first_day) % 7)
    if week == 'fifth':
