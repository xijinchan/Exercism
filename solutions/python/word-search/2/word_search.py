...
class WordSearch:
    ...
    def search(self, word):        
        ...
        def search_diagonal(start, step):
            indices = [(start[0]+i*step[0], start[1]+i*step[1]) for i in range(len(word))]
            if all(0 <= indices[i][0] < len(self.puzzle) and 0 <= indices[i][1] < len(self.puzzle[0])
                   and self.puzzle[indices[i][0]][indices[i][1]] == word[i] for i in range(len(word))):
                return (Point(start[0], start[1]), Point(start[0]+(len(word)-1)*step[0], start[1]+(len(word)-1)*step[1]))
        ...
        # search_diagonal() within bounds
        for row in range(len(self.puzzle)):
            for k in range(len(self.puzzle[row])):
                output = search_diagonal((row, k), (1, 1))
                if output: return output
                output = search_diagonal((row, k), (-1, -1))
                if output: return output
                output = search_diagonal((row, k), (-1, 1))
